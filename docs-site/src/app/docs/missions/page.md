...
A mission is a large initiative broken into a sequence of shippable features. Each feature gets its own branch and PR. State persists across Claude Code sessions via files in `.tstack/`. {% .lead %}
...
Creates `plan.md` for the current feature. See the [command reference](/docs/commands) for the plan.md format.

The plan goes in `.tstack/features/<N>/plan.md`.

### `/execute`

Reads `plan.md`, creates an Agent Team, runs implementation, marks tasks complete. See [Agents](/docs/agents) for coordination details.
