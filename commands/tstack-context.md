...
```
.claude/agents/tstack-*.md
~/.claude/agents/tstack-*.md
```

Map the task domains:
- Backend/Convex changes → `tstack-convex` (if available)
- UI/component changes → `tstack-ui` (if available)
- Both → both agents with coordination needed
- Neither domain → implement in main context or use `tstack-implementer`
...
### Available agents
- tstack-convex / tstack-ui / tstack-implementer / main context
