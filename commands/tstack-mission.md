...
- How to proceed: `/tstack-scope` → `/tstack-plan` → `/tstack-execute` → `/tstack-next`
...
> "Mission started. Feature 1: <name>. Run `/tstack-scope` to research it, then `/tstack-plan` to plan it, then `/tstack-execute` to build it. Or run `/tstack-next` after building to ship + advance."
